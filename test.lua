-- Modül: Jarvis Ana Kontrol Menüsü V14.1 (Sabitlenmiş Arayüz)
...
frame.Size = UDim2.new(0, 270, 0, 750) 
...
title.Text = "JARVIS V14.1 [DATA HARVEST]"
...
-- UI Düzeltmesi: Otomatik kaydırma boyutu eklendi!
...
if hookmetamethod then
    ...
    OldNameCall = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        if isSniffing and not checkcaller() and method == "FireServer" and self:IsA("RemoteEvent") then
            local args = {...}
            local paket = { Remote = self.Name, Zaman = os.date("%X"), Argumanlar = args }
...
btnKopyala.MouseButton1Click:Connect(function()
    if #yakalananVeriler == 0 then
        btnKopyala.Text = "⚠️ KAYIT YOK! ÖNCE BİR ŞEYLER YAPIN"
        task.wait(1.5)
    ...
    if setclipboard then
        setclipboard(döküm)
        btnKopyala.Text = "✅ KOPYALANDI! BANA GÖNDERİN!"
...
local function VeriCevir(veri)
    if veri == "" or veri == "nil" then return nil end if veri == "math.huge" then return math.huge end
    if tonumber(veri) then return tonumber(veri) end if veri:lower() == "true" then return true end
...
btnAtesle.MouseButton1Click:Connect(function() local h = GecerliHedef() if h then h:FireServer(VeriCevir(txtArg1.Text), VeriCevir(txtArg2.Text), VeriCevir(txtArg3.Text)) end end)
...
btnSpam.MouseButton1Click:Connect(function()
    spam_aktif = not spam_aktif local h = GecerliHedef()
    if spam_aktif and h then btnSpam.Text = "SPAM AKTİF! (DURDURMAK İÇİN BAS)" btnSpam.BackgroundColor3 = Color3.fromRGB(220, 0, 0)
...
print("Jarvis V14.1: Arayüz Düzeltmesi Aktif.")
